...
class IntuitionBehindFormula(MovingCameraScene):
    def construct(self):
        ...
        arrow3 = Arrow(formula2_box.get_bottom(), buff=0.1)
        ...
        arrow5 = Arrow(formula3_box.get_bottom(), DOWN*3, buff=0.1)
        self.play(Create(arrow5))
        self.play(Create(formula3_box), Write(formula3))

        # Move camera further downwards
        self.play(self.camera.frame.animate.move_to(DOWN*13))

        # Write fourth equation and box
        formula4 = MathTex(r"F^{\alpha}(F(t))=F^{\alpha}\left\{A_n.\psi_n \right\}=\sum_{1}^{\infty}A_n.\lambda_n^{\alpha}.\psi_n").scale(1.2)
        formula4_box = SurroundingRectangle(formula4, buff=0.1)
        formula4_group = VGroup(formula4_box, formula4)
        formula4_group.next_to(arrow4, DOWN)
